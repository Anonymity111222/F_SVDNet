...
from models.F_SVD_Net import F_SVD_Net_3,F_SVD_Net_3_Conv_SVD,F_SVD_Net_3_FFT_Conv
class Exp_ETTh(Exp_Basic):
    ...
    def _build_model(self):
        ...
        model = F_SVD_Net_3(self.args.modes, self.args.seq_len, self.args.pred_len,self.args.F_channels)
    ...
    def valid(self, valid_data, valid_loader, criterion):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(valid_loader):
            pred, pred_scale, true, true_scale = self._process_one_batch_SCINet(
    ...
    def test(self, setting, evaluate=False):
        ...
        for i, (batch_x,batch_y,batch_x_mark,batch_y_mark) in enumerate(test_loader):
            ...
            # FFT_pred.append(torch.fft.rfft(pred,dim=1).detach().cpu().numpy())
            # FFT_True.append(torch.fft.rfft(true,dim=1).detach().cpu().numpy())
            # FFT_pred_scales.append(torch.fft.rfft(pred_scale,dim=1).detach().cpu().numpy())
            # FFT_True_scales.append(torch.fft.rfft(true_scale,dim=1).detach().cpu().numpy())
        ...
        # FFT_pred = FFT_pred.reshape(-1, FFT_pred.shape[-2], FFT_pred.shape[-1])
        # FFT_True = FFT_True.reshape(-1, FFT_True.shape[-2], FFT_True.shape[-1])
        # FFT_pred_scales = true_scales.reshape(-1, FFT_pred_scales.shape[-2], FFT_pred_scales.shape[-1])
        # FFT_True_scales = FFT_True_scales.reshape(-1, FFT_True_scales.shape[-2], FFT_True_scales.shape[-1])
        ...
        if self.args.save:
            ...
            # np.save(folder_path + 'FFT_pred.npy', FFT_pred)
            # np.save(folder_path + 'FFT_True.npy', FFT_True)
            # np.save(folder_path + 'FFT_pred_scales.npy', FFT_pred_scales)
            # np.save(folder_path + 'FFT_True_scales.npy', FFT_True_scales)
