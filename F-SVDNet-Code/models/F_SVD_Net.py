...
class FFT_Conv(nn.Module):
    def __init__(self, modes, channels, modes1,modes2 ):
        super(FFT_Conv, self).__init__()

        self.modes = modes
        self.channels = channels
       
        self.W_FFT1 = FFT1D_block(self.channels, self.channels, modes1,modes2)
...
class F_SVD_Net_3_FFT_Conv(nn.Module):
    def __init__(self, modes, channels,  pred_num, F_channels):
        super(F_SVD_Net_3_FFT_Conv, self).__init__()

        self.modes = modes
        self.channels = channels
        self.svd_block_1 = FFT_Conv(self.channels, self.channels, modes,modes)
        self.svd_block_2 = FFT_Conv(self.channels, self.channels, modes,modes)
        self.W_FFT_out = FFT1D_block(self.channels, self.channels, modes,modes, pred_num)
    def forward(self, x):
        out_1 = self.svd_block_1(x)
        out_2 = self.svd_block_2(out_1)
...
class Conv_SVD(nn.Module):
    def __init__(self, modes, channels, modes1,modes2, F_channels):
        super(Conv_SVD, self).__init__()

        self.modes = modes
        self.channels = channels
       
        self.W_SVD = SVD_Block_3(self.channels, self.channels, modes1,modes2, F_channels)
        self.conv = nn.Conv1d(channels,channels,3,1,1)
        self.relu = nn.ReLU()
    def forward(self, x):
        conv_x = self.conv(x)
...
class F_SVD_Net_3_Conv_SVD(nn.Module):
    def __init__(self, modes, channels,  pred_num, F_channels):
        super(F_SVD_Net_3_Conv_SVD, self).__init__()

        self.modes = modes
        self.channels = channels
        self.svd_block_1 = Conv_SVD(self.channels, self.channels, modes,modes, F_channels)
        self.svd_block_2 = Conv_SVD(self.channels, self.channels, modes,modes, F_channels)
